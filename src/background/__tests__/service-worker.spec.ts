...
describe('chrome.runtime.onInstalled listener', () => {
  ...
  it('should log versions and call openWhatsNewPage when there is an update from version 1.4.0', () => {
    ...
    // Trigger the onInstalled event with UPDATE reason and previous version 1.4.0
    onInstalledListener({
      reason: 'update',
    ...
    expect(console.info).toHaveBeenCalledWith('Current Version: 1.4.1');

    // Check that openWhatsNewPage is called for the update
    expect(chrome.tabs.create).toHaveBeenCalled();
  });

  it('should NOT log versions and call openWhatsNewPage when there is no update', () => {
    jest.spyOn(chrome.runtime, 'getManifest').mockReturnValue({
      version: '1.4.0',
      manifest_version: 3,
      name: '',
  ...
  it('should NOT log versions and call openWhatsNewPage when the previous version is not 1.3.0', () => {
    jest.spyOn(chrome.runtime, 'getManifest').mockReturnValue({
      version: '1.4.0',
      manifest_version: 3,
      name: '',
  ...
  it('should NOT log versions and call openWhatsNewPage when the previous version the same as the current version', () => {
    jest.spyOn(chrome.runtime, 'getManifest').mockReturnValue({
      version: '1.4.0',
      manifest_version: 3,
      name: '',
